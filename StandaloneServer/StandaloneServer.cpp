...
static void MP_Cfg(int &ctx, int &send3d, int &restore3d, int &selffirst,
	...
	if (s_ctx < 0) {
		...
		Log("[MP-CFG] v131 ctx=%d (%s) send0x3D=%d restore0x3D=%d selfSpawnFirst=%d moveAsSpawn=%d rebuildMove=%d smoothMove=%d",
...
int MP_PathMode() {
	...
	if (s_mode < 0) {
		...
		Log("[MP-CFG] v131 pathMode=%d (8th char of mp_ctx.cfg)", s_mode);
	}
	return s_mode;
}

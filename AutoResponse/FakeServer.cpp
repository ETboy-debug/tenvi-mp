...
thread_local TenviAccount TA; // [MP]  FakeServer.h: 
...
struct RemotePlayer {
	...
	std::set<int> born_to; // [v184] targets that already received a one-time 0x11 birth in smooth mode
};
...
void PlayerStatPacket(TenviCharacter &chr) {
	...
	sp.Encode2(chr.stat_mp); // 004957D7, mb (MP)
	...
	sp.Encode2(189); // 004958F7, NeBJ
	sp.Encode2(2279); // 0049591F, @NeBJ

	if (GetRegion() == TENVI_KRX) {
		sp.Encode2(0);
	}

	sp.Encode2(131); // 00495947, sXs[h
	sp.Encode2(100); // 0049596F, sXs[h
	sp.Encode2(22); // 00495997, R
	sp.Encode2(23); // 004959B3, XR
...
void WorldMapUpdatePacketTest(BYTE area_code) {
	...
	activated_area.push_back(8); // V@ACh
	activated_area.push_back(2); // uACh
	activated_area.push_back(5); // ^[B1ACh
	activated_area.push_back(6); // ~mXACh

	if (GetRegion() != TENVI_HK) {
		activated_area.push_back(1); // rLEBj[ACh
		activated_area.push_back(3); // t@gACh
...
//    v141 had ctx=1 right but shipped the v140 DLL, which is why it failed.
	...
	if (false && MP_SmoothMove() && op == 0x0C && len >= 23 && pkt[1] == 0x01) {
		BYTE nodes = pkt[2];
	...
	// [v183] REAL smooth: CP 0x19 (high-freq absolute coords, already parsed
	// above into nx,ny at line 1011) -> downlink 0x19 relative delta.
	// Client 0x19 handler 0x48D4EA:
	//   Dec4(oid) -> PathDecode([count:1][int16*count]) -> Dec1(stance)
	//   -> LookupByOID -> SetMoving/SetMovePath/SetStance. count = #int16.
	// 2D move needs dx,dy => count=2 -> packet = 11B:
	//   [0x19][oid:4 LE][count=2][dx:int16 LE][dy:int16 LE][stance:1]
	// [v185] Some clients/characters emit CP 0x0C for actual movement while
	...
	if (MP_SmoothMove() && (op == 0x19 || op == 0x0C) && len >= 9) {
		short dx = 0, dy = 0; BYTE stance = 0; bool moved = false;
		{
			...
			if (uit != g_players.end()) {
				...
				if (p.has_last_move) {
					...
					long ldx = (long)fdx; if (ldx < -32768) ldx = -32768; else if (ldx > 32767) ldx = 32767;
					long ldy = (long)fdy; if (ldy < -32768) ldy = -32768; else if (ldy > 32767) ldy = 32767;
		...
		if (!moved) return;
		// [v187] real move pose comes from parsed CP 0x0C/0x19; hard-coded stand direction froze the avatar.
		if (op == 0x0C && mv_stance) stance = mv_stance;
		else if (op == 0x19 && len >= 35 && pkt[34]) stance = pkt[34];
		else stance = 7;
		mv_dir = (dx < 0) ? 0 : 1;
		// [v184] One-time birth: a 0x11 spawn (no 0x12 remove) registers the oid
		// on the receiver BEFORE any 0x19 move is applied, so the first 0x19 always
		// hits a registered object. Remote avatars are server-driven (no client
		// physics), so relative 0x19 deltas cannot drift -> a periodic rebuild/
		// resync is unnecessary and would only add a visible flash. Removed.
		for (auto &t : targets) {
			...
			// [v184] smooth in-place move. Decoder verified (0x45CAEE): reads N
			// then loops N int16 PathDecodeInner calls, so count=2 (dx,dy) is
			// exactly the wire format the handler expects. Handler 0x48D4EA is
			// null-safe (oid miss -> skip, no crash).
			ServerPacket sp;
			sp.Encode1(0x19);
			sp.Encode4(me.id);
			sp.Encode1(0x02);
		...
		MP_MARK("MP-FWD v184 smooth-delta-0x19");
		return;
	}
	...
	if (MP_RebuildMove() && has_pos) {
		...
		// the hop size can shrink: 60px hops looked like teleporting (""),
		...
		// [v130] Back to the best user-tested rebuild params (V126: ""):
		...
		for (auto &t : targets) {
			...
			// avatar visibly blinked every hop ("").
			...
			// second character morphs into the first ("21"). Mirrors
	...
	// [v186] DEFAULT RELAY + opcode sniffer.
	// Every game packet we do not specially handle (emote / attack / skill /
	// chat / player-shop flag / NPC talk / ...) reaches here. Movement (0x0C /
	// 0x19) is excluded above. We log a histogram so the shop/emoji opcodes can
	// be identified, and (when MP_RawRelay() is ON via the 8th cfg char) forward
	// the packet verbatim to same-map peers for the client to render.
	if (op != 0x0C && op != 0x19) {
		static std::map<BYTE, int> g_relayHist;
		static int g_relayPkts = 0;
		int &c = g_relayHist[op];
		if (c < 2) {
			char hex[48]; int n = (int)(len < 20 ? len : 20);
			for (int i = 0; i < n; i++) sprintf(hex + 2 * i, "%02X", (unsigned char)pkt[i]);
			hex[2 * n] = 0;
			printf("[MP-SNIFF v186] op=%02X len=%d sample=%s targets=%zu\n",
				(unsigned)op, (int)len, hex, targets.size());
		}
		c++; g_relayPkts++;
		if ((g_relayPkts % 200) == 0) {
			printf("[MP-SNIFF v186] histogram:");
			for (auto &kv : g_relayHist) printf(" %02X=%d", kv.first, kv.second);
			printf("\n");
		}
		if (MP_RawRelay() && !targets.empty()) {
			ServerPacket sp;
			sp.Raw(pkt, len);
			for (auto &t : targets) MP_BroadcastToSid(t.sid, sp, MP_RemoteCtx());
			printf("[MP-FWD] v186 rawrelay op=%02X len=%d -> %zu\n",
				(unsigned)op, (int)len, targets.size());
		}
		return;
	}
...
void ChangeMap(TenviCharacter &chr, WORD map_id, float x, float y) {
	...
	// [v94] Collect dirB () targets inside the lock, then send
	...
	std::vector<RemotePlayer> dirA_targets;
	{
		...
		for (auto &kv : g_players) {
			...
			// [v54i] ""** self spawn **( v54h 
			// , t_client  ChangeMap ).  ChangeMap
			//  self spawn , t_client  ChangeMap .
			// [v54c] "":  other_sid 
			...
			// spawn to the already-present player (A) lands with live=0
			// (character not yet in the client hashmap) and the following 0x0C
			// move packets are silently dropped ->  sees  standing
			// still. dirB () got a retry x2 and registers (live!=0) so it
			// CAN move. Give A the same retry so BOTH sides move.
	...
	// [v94] dirB deferred: (t_sid) other.
	// Sleep ;  3.0s  CN  ChangeMap 
	// (: A  B, B  A,  B 
	// ChangeMap ). 0x3D/0x11  50ms, 
	// (retry) .  self spawn , 
	// , .
...
#endif
		WorldSelectPacket();   // ("")
		WorldListPacket();      // (, CP_WORLDLIST, CharacterListPacket_Test)

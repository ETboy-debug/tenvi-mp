...
// [DIAG] Global exception handler - catches ANY crash and logs the address
...
BOOL WINAPI DllMain(HINSTANCE hinstDLL, DWORD fdwReason, LPVOID lpvReserved) {
	if (fdwReason == DLL_PROCESS_ATTACH) {
		...
		SetUnhandledExceptionFilter(CrashHandler);
			// [DIAG] Clear diagnostic log  (V16 marker)
			{ FILE *f = NULL; fopen_s(&f, "D:/mp_diag.log", "w"); if (f) { fprintf(f, "=== DLL attach V16 ===\n"); fclose(f); } }
